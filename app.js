...
function computeStats() {
  ...
  flatSessions.forEach((s) => {
    ...
    if (s.week === 12 && s.day === "za") raceDone = true;
  });
  let streak = 0, run = 0;
  flatSessions.forEach((s) => {
    const e = log[sid(s.week, s.day)];
...
function renderHero(stats) {
  ...
  const mottos = ["Zet 'm op, strijder!", "Lekker bezig, strijder!", "Je bouwt 'm op, strijder.", "Halverwege — doorpakken! ⚡", "Bijna race-klaar, strijder!", "Finisher! Wat een strijder!"];
